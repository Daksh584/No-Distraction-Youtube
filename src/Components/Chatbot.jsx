...
const Chatbot = ({ videoLink, videoTitle, videoTranscript }) => {
  ...
  const handleSendMessage = async () => {
    if (input.trim()) {
      ...
      try {
        ...
        const errorMessage = { sender: 'ai', text: '❌ Failed to fetch AI response. Please try again.' };
  ...
  return (
    <div className="chatbot glass-strong rounded-3xl p-6 w-full shadow-xl animate-fade-in">
      {/* Header */}
      <div className="mb-6">
        <h3 className="font-poppins font-bold text-2xl gradient-text mb-2">AI Learning Assistant</h3>
        <p className="text-sm text-base-content/60">Ask questions about the video content</p>
      </div>
      ...
      <div className="messages mb-6 space-y-4 max-h-[500px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-primary-500 scrollbar-track-base-200">
        ...
        {messages.map((message, index) => (
          <div
            key={index}
            className={`message flex ${message.sender === 'user' ? 'justify-end' : 'justify-start'} animate-slide-up`}
          >
            <div className={`max-w-[80%] ${message.sender === 'user'
              ...
              <div className={`text-sm leading-relaxed ${message.sender === 'user' ? 'text-white' : 'text-base-content'
                }`}>
